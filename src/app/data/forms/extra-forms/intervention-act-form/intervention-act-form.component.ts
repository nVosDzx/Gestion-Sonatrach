...
@Component({
  selector: 'app-intervention-act-form',
  templateUrl: './intervention-act-form.component.html',
  styleUrls: ['./intervention-act-form.component.css']
})
export class InterventionActFormComponent implements OnInit , OnDestroy {
  sub : Subscription ;
  idIntervention:any={};
  key;
  ...
  constructor(private interventionManager : InterventionManagerService,private route : ActivatedRoute,
    ...
    if(this.key) this.interventionManager.getACT(this.key).subscribe(data =>{
  ...
  save(intervention){
    if(this.key)
      {
        this.interventionManager.updateACT(this.key,intervention.value);
      ...
      {
        this.interventionManager.createACT(intervention.value);
