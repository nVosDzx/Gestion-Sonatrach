...
@Component({
  selector: 'app-intervention-am-form',
  templateUrl: './intervention-am-form.component.html',
  styleUrls: ['./intervention-am-form.component.css']
})
export class InterventionAMFormComponent implements OnInit , OnDestroy{
  ...
  constructor(private interventionManager : InterventionManagerService,private route : ActivatedRoute,
    ...
    if(this.key) this.interventionManager.getAM(this.key).subscribe(data =>{
  ...
  save(intervention){
    if(this.key)
      {
        this.interventionManager.updateAM(this.key,intervention.value);
      ...
      {
        this.interventionManager.createAM(intervention.value);
